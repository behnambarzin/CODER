...
class RecursiveEngine:
    def __init__(self, model):
        ...
        self.log_filename = f"chat_history_{self.session_id}.json"
        # ----------------------------------
        self.base_system_prompt = """
        [ROLE]
        You are an Autonomous Technical Agent. Your goal is to achieve mission objectives through precise tool manipulation, logical reasoning, and iterative error correction. You operate in a local environment with no internet access.

        [CAPABILITY MANIFEST]
        You have access to the following tools. Use them with strict Pythonic syntax: `tool_name(key="value")`.

        1. FILE SYSTEM OPERATIONS:
        - `list_files(directory=".")`: List items in a directory.
        - `get_tree(directory=".", max_depth=3)`: Visual directory structure.
        - `read_file(filename="...", start_line=1, end_line=None)`: Read specific lines.
        - `write_file(filename="...", content="...", mode="overwrite")`: Create/replace files.
        - `append_to_file(filename="...", content="...")`: Add to end of file.
        - `replace_in_file(filename="...", search="...", replace="...", use_regex=False)`: Advanced text replacement.
        - `search_files(pattern="...", directory=".")`: Glob search.
        - `grep_search(pattern="...", file_path="...", context_lines=3)`: Pattern search with context.
        - `get_file_stats(filename="...")`: Detailed metadata.

        2. CODE INTELLIGENCE (AST-BASED):
        - `analyze_code_structure(filename="...")`: Map classes, functions, and imports.
        - `extract_logic_block(filename="...", target_name="...", target_type="function")`: Surgically extract a specific function/class.
        - `lint_and_validate(code="...")`: Static analysis for syntax/safety.
        - `trace_dependencies(filename="...")`: Map internal function call graphs.

        3. EXECUTION & SYSTEM:
        - `python_executor(code="...")`: Run arbitrary Python code.
        - `run_file(filename="...")`: Execute a specific .py file.
        - `shell_execute(command="...")`: Low-level system commands.
        - `get_system_info()`: Environment snapshot.
        - `manage_process(action="list"|"kill", pid=None)`: Process orchestration.
        - `scientific_compute(expression="...")`: High-precision math.
        - `ask_user(question="...")`: Request human intervention.

        [OPERATIONAL PROTOCOLS]
        - QUOTE PROTOCOL: To prevent parsing errors, use alternating quotes. If content has double quotes ("), wrap in single quotes ('). Example: `write_file(content='He said "Hello"')`.
        - THE TWO-STEP RULE: For complex/long scripts, do not use `python_executor`. Instead: 1. `write_file` the script to a .py file, then 2. `run_file` that file.
        - ERROR HANDLING: If a tool returns an ERROR, perform a 'Post-Mortem'. Analyze the error, adjust your parameters, and retry. If it fails twice, use `ask_user` or trigger REQUEST_GUIDANCE.

        [REQUIRED OUTPUT ARCHITECTURE]
        You must respond using these exact blocks. Do not include conversational filler (e.g., "Understood", "I will do that").

        [TRAJECTORY: Step X/Y | Task -> Next Target -> Ultimate Objective]
        [SYNTHESIS: Your technical reasoning, error analysis, or REQUEST_GUIDANCE]
        [ACTION: tool_name(args)]
        OR
        [COMPLETION: A final technical summary of the mission results]
        """
        self._initialize_engine()
        
    def save_history(self):
        """Serializes the current chat history to a JSON file for persistence."""
        try:
            with open(self.log_filename, 'w', encoding='utf-8') as f:
                json.dump(self.history, f, indent=4)
        except Exception as e:
            print(f"\n[!] PERSISTENCE ERROR: Could not save history: {e}")
    ...
    def _parse_response(self, content):
        ...
        Uses block-slicing and iterative extraction to ensure 100% reliability 
        even if the LLM includes trailing brackets, extra newlines, or messy formatting.
        """
        ...
        # This regex finds the START of any header block.
        # It matches: [HEADER], HEADER:, [HEADER:, or HEADER -
        header_pattern = r"(?i)\[?\s*(TRAJECTORY|SYNTHESIS|ACTION|COMPLETION)\s*\]?\s*[:\-]\s*"
        
        # Find all header occurrences and their positions
        matches = list(re.finditer(header_pattern, content))
        
        for i, match in enumerate(matches):
            header_name = match.group(1).upper()
            header_key = headers_map[header_name]
            
            # Determine the boundaries of this block
            start_index = match.end()
            end_index = matches[i+1].start() if i + 1 < len(matches) else len(content)
            
            # Extract and "sanitize" the block content
            # We strip trailing brackets/parentheses that LLMs often add when closing a block
            block_content = content[start_index:end_index].strip().rstrip(' ]\n\r')
            
            if header_key == "action":
                # The ACTION block is special. We need to split the tool name from the args.
                # We use re.DOTALL so that the (args) part can span multiple lines.
                call_match = re.search(r"(\w+)\s*\((.*)\)", block_content, re.DOTALL)
                ...
                if call_match:
                    ...
                    # Fallback: If no parentheses are found, the whole block is treated as the tool name
                    # This handles cases like: [ACTION]: list_files
                    res["action"] = (block_content, "")
            else:
                # For roadmap, thought, and final, just take the cleaned block
                res[header_key] = block_content
    ...
    def run_task(self, user_input):
        ...
        try:
            ...
            # This executes NO MATTER WHAT (Success, Error, or Critical)
            self.save_history()
...
if __name__ == "__main__":
    ...
    def open_logs_directory():
        """Opens the current working directory in the OS file explorer."""
        path = os.getcwd()
        try:
            if os.name == 'nt':  # Windows
                os.startfile(path)
            ...
            print(f"\n{ui.LIGHT_GREY}Directory requested. Opening explorer...{ui.RESET}")
            time.sleep(1)
        except Exception as e:
            print(f"\n{ui.log_error(f'Failed to open directory: {e}')}")
    ...
    try:
        ...
        while True:  # <--- This keeps the session alive
            ...
            if goal.strip():
                ...
                print(f"{ui.DIM}4. [OPEN LOG DIRECTORY] (View all chat histories){ui.RESET}")
                ...
                if choice == '1':
                    ...
                    time.sleep(0.5)
                    new_goal = input(f"{ui.BOLD}ENTER NEW DIRECTIVE ❯ {ui.RESET}")
                    engine = RecursiveEngine(selected_model) 
                    engine.history = [{"role": "system", "content": engine.base_system_prompt}, {"role": "user", "content": f"NEW MISSION: {new_goal}"}]
                    engine.current_objective = new_goal
                    continue
