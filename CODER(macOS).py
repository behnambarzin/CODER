...
class Toolbox:
    ...
    def list_files(directory=".", **kwargs):
        ...
        REWRITTEN: Added macOS .DS_Store and metadata filtering to prevent context pollution."""
        try:
            ...
            report = []
            # macOS specific ignore list to prevent LLM confusion
            ignored_metadata = {'.DS_Store', '.localized', '.AppleDouble', '.Trashes'}
            
            for item in items:
                if item in ignored_metadata: continue
            ...
            return f"SUCCESS: Found {len(report)} relevant items in '{target}':\n" + "\n".join(report)
    ...
    def get_tree(directory=".", max_depth=3, **kwargs):
        ...
        REWRITTEN: Added macOS/Git metadata filtering for cleaner tree output."""
        def _build_tree(path, prefix="", depth=0):
            if depth > max_depth: return ""
            tree = ""
            try:
                # Filter out macOS and Git junk to keep the tree concise for the LLM
                ignored = {'.DS_Store', '.git', '.localized'}
                items = sorted([i for i in os.listdir(path) if i not in ignored and not i.startswith('.')])
    ...
    def get_system_info():
        ...
        REWRITTEN: Fixed Apple Silicon architecture detection and user retrieval for macOS."""
        import platform, psutil, getpass
        try:
            # On Mac, platform.processor() is unreliable. machine() provides the truth (arm64 vs x86_64).
            ...
            info = {
                ...
                "cwd": os.getcwd(),
                "user": getpass.getuser(), # More reliable than os.getlogin() on macOS
                "is_apple_silicon": "arm64" in arch.lower() or "aarch64" in arch.lower()
            }
            try:
                import psutil
    ...
    def shell_execute(command):
        ...
        REWRITTEN: Forced ZSH execution to ensure macOS environment/path compatibility."""
        try:
            # On Mac, running through /bin/zsh ensures that the agent's 
            # shell commands respect the user's actual path and environment.
            shell_executable = "/bin/zsh" if os.name != 'nt' else None
    ...
    def manage_process(action, pid=None, **kwargs):
        ...
        REWRITTEN: Switched from Windows tasklist/taskkill to POSIX ps/kill."""
        try:
            if action == "list":
                import subprocess
                # Use 'ps' with specific flags for a clean, readable output on macOS/Unix
                cmd = "ps -eo pid,comm | head -n 20" if os.name != 'nt' else "tasklist"
                res = subprocess.check_output(cmd, shell=True, text=True)
                return f"SUCCESS: Process List (Top 20):\n{res}"
            elif action == "kill":
                ...
                try:
                    ...
                    if os.name == 'nt':
                        ...
                        # macOS/Unix standard SIGKILL
                        os.kill(pid_int, signal.SIGKILL)
                    return f"SUCCESS: Process {pid_int} terminated."
                except (ValueError, TypeError): return "ERROR: PID must be an integer."
...
class LangChainEngine:
    ...
    def _get_system_prompt(self) -> str:
        ...
        [ENVIRONMENTAL CONTEXT: macOS/DARWIN]
        - OS TYPE: Unix-based (macOS).
        - SHELL: zsh.
        - COMMANDS: Use POSIX standard commands (e.g., 'ls' not 'dir', 'rm' not 'del', 'cat' not 'type').
        - PATHS: Always use forward slashes (/) for file paths.
        - CASE SENSITIVITY: The filesystem is case-insensitive but case-preserving. 
          Treat 'File.py' and 'file.py' as the same file to avoid logical errors.
