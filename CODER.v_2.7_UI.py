...
class RichUI:
    ...
    def startup_sequence(self):
        self._force_clear()
        welcome = Text("CODER v2.7", style="primary|bold")
...
class MemoryManager:
    ...
    def query_wisdom(self, query_text: str, n_results: int = 3) -> str:
        """Retrieves only the most relevant past LESSONS using semantic search."""
        if self.model is None:  # SAFETY CHECK: If model didn't load, return empty string
            return ""
            
        query_embedding = self.model.encode(query_text).tolist()
        
        try:
            # UPDATED: Added 'where' filter to strictly retrieve only documents 
            # tagged with type='lesson'. This prevents old "experience" blocks 
            # from polluting the new UI.
            ...
            for doc in results['documents'][0]:
                # Clean up any accidental whitespace/newlines from the stored lesson
                clean_doc = doc.strip()
                if clean_doc:
                    wisdom_blocks.append(f"- {clean_doc}")
                
            return "\n".join(wisdom_blocks)

        except Exception as e:
            # If a filter error occurs (e.g., no 'type' metadata exists in old records), 
            # we fall back to an empty string rather than crashing the agent.
            return ""
...
class LangChainEngine:
    ...
    def node_reflect(self, state: AgentState) -> Dict[str, Any]:
        with self.ui.node_context("REFLECT"):
            self.ui.update_status("Optimizing cognitive weights...")
            history_str = "\n".join([f"{m['role']}: {m['content']}" for m in state['history']])
            
            # The prompt is already optimized to return ONLY the lesson text
            reflection_prompt = (
                "You are a Meta-Cognitive Optimizer. Analyze the mission history provided. "
                "Extract only the single most important lesson learned from this experience. "
                "Output ONLY the lesson text itself. Do not include any headers, prefixes like 'LESSON:', or metadata.\n\n"
                f"MISSION HISTORY:\n{history_str}"
            )
            ...
            try:
                ...
                # --- FIX: Removed the 'if "EXPERIENCE:" in wisdom' check ---
                # Since we no longer use that header, we check if the string is simply not empty.
                if wisdom:
                    self.memory.store_wisdom(wisdom, {
                        "type": "lesson",  # UPDATED: Changed type from 'experience' to 'lesson'
