...
class MemoryManager:
    """Handles local vector storage and semantic retrieval."""
    def __init__(self, embedding_path: str, db_path: str):
        ...
        if not self.embedding_path.exists():
            print(f"  [WARNING] Embedding model not found at {self.embedding_path}. Memory will be DISABLED.")
            self.model = None
        else:
            print(f"  [SYSTEM] Loading local embeddings...")
            self.model = SentenceTransformer(str(self.embedding_path))
        
        self.client = chromadb.PersistentClient(path=str(self.db_path))
        self.collection = self.client.get_or_create_collection(self.collection_name)

    def store_wisdom(self, wisdom_text: str, metadata: Dict[str, Any]):
        if self.model is None: return # Silent skip if memory is disabled
        embedding = self.model.encode(wisdom_text).tolist()
        # Use timestamp as ID to ensure uniqueness
        doc_id = f"wisdom_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
        
        self.collection.add(
            ids=[doc_id],
            embeddings=[embedding],
            documents=[wisdom_text],
            metadatas=[metadata]
        )
...
class LLMInterface:
    ...
    def generate_response_streamed(self, history: List[Dict[str, str]], user_input: str, ui: Any, memory_context: str = "") -> 'AgentResponse':
        ...
        # Re-build the prompt template dynamically to ensure memory is at the top of the system instructions
        ...
        try:
            ...
            try:
                # Improved: Find all JSON-like structures and take the LAST one 
                # (local models often repeat themselves or provide a summary after the JSON)
...
class LangChainEngine:
    ...
    def node_llm(self, state: AgentState) -> Dict[str, Any]:
        ...
        # PATCH: Truncate last_observation to prevent context window explosion
        obs = state.get('last_observation', '')
        if len(obs) > 5000:
            obs = obs[:5000] + "... [TRUNCATED FOR CONTEXT SAFETY]"
        ...
        try:
            ...
            # FIXED: Use list(state["history"]) to create a shallow COPY. 
            # This prevents mutating the state in-place.
            new_history = list(state["history"])
            new_history.append({"role": "assistant", "content": clean_assistant_message})
            
            if len(new_history) > 13:
    ...
    def node_reflect(self, state: AgentState) -> Dict[str, Any]:
        self.ui.log_node_transition("reflect")
        self.ui.log_thought("Performing deep-layer post-mortem for cognitive optimization...")
        
        history_str = "\n".join([f"{m['role']}: {m['content']}" for m in state['history']])
        reflection_prompt = (
            "You are a Meta-Cognitive Optimizer. Analyze the mission history provided. "
            "Do not just create a rule. Create a 'High-Density Experience'.\n\n"
            "Format your response exactly as follows:\n"
            "EXPERIENCE: [Brief context of the situation] | ACTION: [What was done] | RESULT: [What happened] | LESSON: [The generalized technical takeaway]\n\n"
            f"MISSION HISTORY:\n{history_str}"
        )

        try:
            response = self.llm_interface.llm.invoke(reflection_prompt)
            wisdom = response.content.strip()
            if wisdom and "EXPERIENCE:" in wisdom:
                # Store as an experience type for better retrieval later
                self.memory.store_wisdom(wisdom, {
                    "type": "experience", 
                    "mission": state['mission'],
                    "success": state.get("is_finished", False)
                })
            return {"next_step": "complete"}
        except Exception as e:
            return {"next_step": "plan", "last_observation": f"Reflection failed: {e}"}
    ...
    def node_action(self, state: AgentState) -> Dict[str, Any]:
        ...
        # --- ENHANCED ERROR DETECTION IN OBSERVATION ---
        if "ERROR" in str(observation).upper():
            observation = f"!!! TOOL EXECUTION FAILURE !!!\n{observation}"

        # Safety cap to prevent context window overflow
    ...
    def run_mission(self, mission: str, previous_state: Optional[Dict[str, Any]] = None):
        ...
        if memory_context:
            self.ui.log_observation(f"Retrieved relevant wisdom from memory:\n{memory_context}")

        # If previous_state exists, we merge it to maintain history and trajectory
        if previous_state:
            ...
            # Ensure we don't carry over 'is_finished' from the last mission
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 2. MISSION LOOP
        last_known_state = None # Track state for continuity
        ...
        while True:
            ...
            if goal.strip():
                # Pass the last known state if it exists
                final_state = engine.run_mission(goal, previous_state=last_known_state)
                last_known_state = final_state # Save the state for the next cycle
                ...
                if choice == '1':
                    engine = LangChainEngine(selected_model, ui, tool_manager)
                    last_known_state = None # Wipe continuity
                    continue
                elif choice == '2':
                    continue # last_known_state is preserved, engine.run_mission will use it
                elif choice == '3':
                    selected_model = ui.select_model()
                    engine = LangChainEngine(selected_model, ui, tool_manager)
