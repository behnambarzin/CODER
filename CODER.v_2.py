...
class LangChainEngine:
    ...
    def node_reflect(self, state: AgentState) -> Dict[str, Any]:
        ...
        self.ui.log_thought("Analyzing mission history for strategic optimization...")
        
        history_str = "\n".join([f"{m['role']}: {m['content']}" for m in state['history']])
        reflection_prompt = (
            "You are a meta-cognitive optimizer. Review the following mission history. "
            "Identify ONE high-density strategic rule or 'lesson learned' that would "
            "prevent errors or increase efficiency in future missions. "
            "Format your response as a single, concise sentence. "
            "Start the sentence with 'RULE: '.\n\n"
            f"HISTORY:\n{history_str}"
        )
        ...
        try:
            ...
            if wisdom:
                # Store the lesson in ChromaDB
                self.memory.store_wisdom(wisdom, {"type": "strategic_rule", "mission": state['mission']})
            
            return {"next_step": "complete"} # Move to terminal node
        except Exception as e:
    ...
    def run_mission(self, mission: str):
        ...
        # --- NEW: PRE-FLIGHT MEMORY RETRIEVAL ---
        # Before starting, we ask the memory what it knows about this mission
